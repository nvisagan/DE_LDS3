...
- An important part of code reviews is helping write comments. Sometimes people forget to comment which helps people read and understand code. 
  When another person looks at your code and doesn't understand, its time for a comment. If it's still not understandable, then it's time to clean the code.
  
- It’s easier to manage with containers since they act as virtual machines running certain tasks under a single host.
  Containers bundle their own software and communicate through their own channels.
  Docker can host those containers to help keep track. 
